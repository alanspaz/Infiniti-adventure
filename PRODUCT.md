...
Done in engine/app: settings shell (verbosity, provider kind, **remote base URL + optional model**, API key), character sheet engine + Character sheet UI, dice/resolution engine + Dice UI, save/load shapes, playstyle packs (Hearthlight / Ash Ledger), identity create UI (pack → identity), narrator provider stub (offline pack-template/canned; remote OpenAI-compatible HTTP when configured + stub fallback), maps/whereAmI (starter Embervale graph + Map screen), stills provider stub + **stills UI/cache** (StillFrame, Stills gallery, PersistStore cache; Scene show-me), PersistStore AsyncStorage adapter, **Scene adventure loop** (Home Continue / New scene).
Out of scope until ticketed: companion emotion, multiplayer, AI vendor brand selection, Expo device playtest.
